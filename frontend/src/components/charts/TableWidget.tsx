import { useMemo, useState } from 'react'

interface Props {
  data: Record<string, unknown>[]
}

export function TableWidget({ data }: Props) {
  const columns = useMemo(() => (data[0] ? Object.keys(data[0]) : []), [data])
  const [sortKey, setSortKey] = useState<string | null>(null)
  const [asc, setAsc] = useState(true)

  const rows = useMemo(() => {
    if (!sortKey) return data
    return [...data].sort((a, b) => {
      const av = a[sortKey] as never
      const bv = b[sortKey] as never
      if (av === bv) return 0
      return (av > bv ? 1 : -1) * (asc ? 1 : -1)
    })
  }, [data, sortKey, asc])

  if (!columns.length) return <p className="text-slate-400">Nəticə yoxdur.</p>

  return (
    <div className="overflow-auto rounded-xl border border-slate-700">
      <table className="w-full text-left text-sm">
        <thead className="bg-slate-800 text-slate-300">
          <tr>
            {columns.map((c) => (
              <th
                key={c}
                onClick={() => {
                  if (sortKey === c) setAsc(!asc)
                  else { setSortKey(c); setAsc(true) }
                }}
                className="cursor-pointer px-3 py-2 hover:text-brand"
              >
                {c}{sortKey === c ? (asc ? ' ▲' : ' ▼') : ''}
              </th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className="border-t border-slate-700 hover:bg-slate-800/50">
              {columns.map((c) => (
                <td key={c} className="px-3 py-2 text-slate-200">{String(row[c])}</td>
              ))}
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  )
}
