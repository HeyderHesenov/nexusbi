  ...
  return (
    <div className="space-y-2.5 rounded-xl border border-line bg-surface-2 p-4">
      <div className="flex items-center gap-2">
        ...
        <p className="eyebrow text-ink-soft">Mənşə (lineage)</p>
      </div>
      {empty ? (
        <p className="text-sm text-ink-faint">Mənşə məlumatı çıxarılmadı.</p>
      ) : (
        <>
          <Row icon={<Table2 size={13} />} label="Cədvəllər" items={lineage.tables} />
          <Row icon={<Columns3 size={13} />} label="Sütunlar" items={lineage.columns} />
          <Row icon={<Tag size={13} />} label="Metriklər" items={lineage.metrics} />
        </>
      )}
    </div>
  )
