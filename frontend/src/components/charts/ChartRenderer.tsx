...
export function ChartRenderer({ data, config, height = 320, showLegend = false }: Props) {
  if (!data.length) {
    return <p className="text-ink-soft">Nəticə tapılmadı.</p>
  }
  switch (config.chart_type) {
    case 'bar':
      return <BarChartWidget data={data} config={config} height={height} />
    case 'line':
      return <LineChartWidget data={data} config={config} height={height} />
    case 'area':
      return <AreaChartWidget data={data} config={config} height={height} />
    case 'pie':
      return <PieChartWidget data={data} config={config} height={height} showLegend={showLegend} />
    case 'scatter':
      return <ScatterChartWidget data={data} config={config} height={height} />
    case 'kpi_card':
